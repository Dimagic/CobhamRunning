...
public class RunningTestController implements MsgBox {
	...
	@FXML
	private TableView<CobhamSystem> tSysToRun;
	@FXML
...
//		try {
			if (testCheck.isSelected()) {	
				mainApp.setSystemData(new CobhamSystem("DM00031D", "GZN5", "00-14-B1-01-9B-DC", this));  
				mainApp.setSystemData(new CobhamSystem("USRVR", "NONE", "00-05-35-06-95-77", this));
				mainApp.setSystemData(new CobhamSystem("DM00031D", "GZN9", "00-14-B1-01-9B-AD", this));
				tSysToRun.setItems(mainApp.getSystemData());
		
			} else {
				List<String> currSys = MsgBox.msgScanSystemBarcode();
				String currMac = MsgBox.msgScanMac();
...
//					cobhamSystem.articleProperty().getValue(), cobhamSystem.asisProperty().getValue()));
	...
	private void initItemMenu(){
		...
		mDel.setOnAction((ActionEvent event) -> {
		    mainApp.getSystemData().remove(tSysToRun.getSelectionModel().getSelectedItem());
		    tSysToRun.refresh();
		});
